...
export default function OTPVerificationAttendence() {
		...
		try {
			...
			firebase.auth().signInWithCredential(credentials).then(async (userCrdential) => {
				...
				toast({
					...
					description: `Attendence marked as visited.`,
					variant: "success",
					duration: 5000
				  })
	...
	const sendOtp = async ({ phone }: { phone: string }) => {
		try {
			...
			firebase.auth().signInWithPhoneNumber(phone, verifier).then((confirmationRes) => {
				verificationData.setUserData({ ...verificationData.userData, verificationId: confirmationRes.verificationId })
	...
	const handleResend = () => {
		...
		sendOtp({ phone: verificationData.userData.phone || "" })
	}
				...
				<Card className="w-full max-w-md border-0 shadow-lg">
					<CardHeader className="space-y-3">
						...
						<div className="space-y-1">
							...
							<CardDescription>Enter the 6-digit code sent to your phone number ending in +91xxxxxx{verificationData.userData.phone?.slice(8)}</CardDescription>
						</div>
					</CardHeader>
					<CardContent>
						{!isVerified ? (
