...
doctorRouter.patch(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      // Insert new time slots based on the provided data
      const newTimeSlots = Object.entries(scheduleData).flatMap(
        ([day, slots]: any) =>
          slots.map((slot: any) => ({
            start: slot.start,
            end: slot.end,
            // day: day.charAt(0).toUpperCase() + day.slice(1), // Capitalize day names (Monday, Tuesday, etc.)
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (!req.file) {
        ...
        // if (prevPres && (prevPres.prescriptionUrl !== "/no-file.pdf" && prevPres.prescriptionUrl !== "/no-file.img")) {
        //   const urlOld = prevPres.prescriptionUrl;
        //   const public_id =
        //     "smartHealth/" + prescriptionUrl.split("smartHealth/")[1].split(".")[0];
        //   console.log(public_id);
        //   cloudinary.uploader.destroy(
        //     public_id, // Replace with the extracted public_id
        //     { invalidate: true },
